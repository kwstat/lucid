% lucid_printing.Rnw
% Time-stamp: <02 Jul 2015 12:53:41 c:/x/rpack/lucid/vignettes/lucid_printing.Rnw>
...
\title{Lucid printing of floating-point vectors}
\author{Kevin Wright}
\maketitle
\thispagestyle{fancy}
...
An alternative to significant digits is the concept of ``effective digits'' 
...
fitted models obtained from the \code{asreml}, \code{lme4}, \code{nlme}, and \code{rjags} 
...
\subsection{Example 1 - Rail data}

The following simple example illustrates use of the \code{vc} function for
identical REML models in the \code{nlme}, \code{lme4}, and \code{asreml} packages.
...
The package \code{rjags} is used to fit a similar Bayesian model inspired by
\cite{wilkinson2014oneway}.
...
vc(c5)
@
Compare these JAGS point estimates and quantiles with the results from \code{lme4}.
<<>>=
m4
ranef(m4)
@
...
\subsection{Example 2 - federer.diagcheck data}

The second, more complex example is based on a model in \cite{federer2003proc}
...
optimizer.  These results are from \code{lme4} version 1.1-7 and are likely to
be different for later versions of \code{lme4}
...
Note.  Numeric matrices are printed with quotes.  Use \code{noquote()} to
