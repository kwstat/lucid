...
The ‘lucid’ package provides a simple function to improve the format of
floating-point numbers for humans. The ‘lucid()’ function is primarily a
*formatting* function similar to ‘round’ and ‘signif’, but output is
always character.

Note: The `lucid` package was created before the `tibble` package. The
`tibble` package now uses a similar style of formatting dataframes.
...
In the short example below, a separate regression line is fit to each of
five trees’ circumference versus age. The default output is difficult to
