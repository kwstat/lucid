...
The `lucid` package provides a function for printing vectors of floating
...
In R, using the `round()` and `signif()` functions can be used to round
to 3 digits of accuracy, but those functions can still print results in
scientific notation and leave much to be desired.
The `lucid` package provides functions to improve the presentation of
...
vc(c5)
```
Compare these JAGS point estimates and quantiles with the results from `lme4`.
```{r}
m4
ranef(m4)
```
...
optimizer.  These results are from `lme4` version 1.1-7 and are likely to
be different for later versions of `lme4`
