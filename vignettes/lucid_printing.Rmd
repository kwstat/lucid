...
1. Humans cannot comprehend more than three digits very easily.
2. We almost never care about accuracy of more than three digits.
...
In R, using the `round()` and `signif()` functions can be used to round to 3
...
print(df1)
```
Which coeficient is zero?  How large is the intercept?
...
```
Extracting information from the sea of numbers in this view is difficult.  The `lucid` function comes to the resucue, simply by adding one more step to the sequence of pipes.
```{r, broom2}
Orange %>% group_by(Tree) %>% do(tidy(lm(age ~ circumference, data=.))) %>% lucid
```
After formatting, information in the table is almost thrown at the reader, reducing the amount of cognitive effort needed for interpretation.
...
### Example 2 - federer.diagcheck data
...
optimizer, while the final two columns are from the \texttt{NelderMead}
...
Note.  Numeric matrices are printed with quotes.  Use `noquote()` to
print without quotes, for example:
```{r }
